...
export class CheckinService {
  ...
  async checkIn(_checkIn: CheckInDto, companyId: number, storeId?: number, customerId?: number) {
    ...
    if (!checkin) {
      ...
      // if (setting && setting.sendCheckinMessage) {
      //   // let result;
      //   // const textMessage = new TextMessage();
      //   // if (setting.autoGeneratedClaimedMessage) {
      //   //   result = await textMessage.sendToCustomerId(companyCustomer.customerId, "You are just checked in at " + setting.store.name);
      //   // } else {
      //   //   result = await textMessage.sendToCustomerId(companyCustomer.customerId, setting.customCheckinMessage);
      //   // }
      //   // if (result) {
      ...
      if (booking) {
        ...
        this.checkinGateway.sendNotifyCheckIn(storeId, "appointment-activity", activity);

      }
      // this.checkinGateway.sendNotifyCheckIn(storeId, "checkinCustomer", companyCustomer);
      // this.checkinGateway.sendNotifyCheckIn(storeId, "updateCheckin", await this.checkinCount(companyId));
      ...
      this.checkinGateway.sendNotifyCheckIn(storeId, "newKey", secretKey);
